						...
						<ul class="nav_ul">
							...
							<li>
								...
								<ul class="low_depth show1">
									...
									<li><a class="selected" href="arti_vac.php">백신별</a></li>
								</ul>
							</li>
  ...
  <section id="section_container" class="section_arti_vac">
    <div class="news_vac">
      ...
      <div class="news_filter_container">
        ...
        <form action="http://www.covac.news/arti_vac_result.php" method="get">
        <div class="search_wrapper display_flex_center">
          <input placeholder="검색하기" type="search" class="search" required="required" name="search" />
      ...
      <div class="news_container">
        <?php
      			...
      			$numRecords = $db->query("select count(*) from Vac_Cl where title like '%{$search}%'")->fetchColumn();
      			...
      			$query = $db->query("select * from Vac_Cl where title like '%{$search}%' order by num desc limit $start,$listSize");
      ...
      <div class="pages_turner">
        ...
        <?php
        			...
        			if ($first > 1) {
        		?>
        			<a href="http://www.covac.news/arti_vac_result.php?page=<?=($first - 1)?>&search=<?=$search?>">&lt;</a>
        		<?php
        			}

        			for($i = $first; $i <= $last; $i++) {
        		?>
        			<a href="http://www.covac.news/arti_vac_result.php?page=<?=$i?>&search=<?=$search?>"><?=$i?></a>
        		<?php
        			}

        			if ($last < $numPages) {
        		?>
        			<a href="http://www.covac.news/arti_vac_result.php?page=<?=($last + 1)?>&search=<?=$search?>">&gt;</a>
        		<?php
        			}
        		?>

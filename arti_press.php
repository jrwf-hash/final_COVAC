...
<html lang="ko">
	...
	<body>
		...
		?>
			...
			if ($first > 1) {
		?>
			<a href="http://www.covac.news/arti_press.php?page=<?=($first - 1)?>">&lt;</a>
		<?php
			}

			for($i = $first; $i <= $last; $i++) {
		?>
			<a href="http://www.covac.news/arti_press.php?page=<?=$i?>"><?=$i?></a>
		<?php
			}

			if ($last < $numPages) {
		?>
			<a href="http://www.covac.news/arti_press.php?page=<?=($last + 1)?>">&gt;</a>
		<?php
			}
		?>
